...
class Labelme:
    ...
    def __init__(self, image_path,json_path,image_type="jpg"):
        # 加载图片-标签文件
        self.label_dict = self.__load_jsons_images(image_path, json_path, image_type)
        # 检查图片-标签文件是否存在，若不存在则删除
        self.__check_label_image()
        
        # 图片-标签 数量
        self.__size = len(self.label_dict.keys())

        # 加载标注文件数据
        self.CLASSES, self.labels = self.__load_labels()
    ...
    def __load_jsons_images(self,image_path,json_path,image_type):
        json_data = {}
